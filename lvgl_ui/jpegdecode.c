...
JpegDecoder* JpegDecoderCreate(int flags){
...
ImgFrame *JpegDecoderGetFrame(JpegDecoder* v){
	...
	long long b = GetNowUs();
	long long c = GetNowUs();
	...
	long long d = GetNowUs();

    TLOGD("Request Video Stream Buffer ok %p %d %p %d",buf,buflen,ringBuf,ringBufLen);
    ...
    if(buflen >= dataLen)
    {
    	TLOGD("%d %p %p %d %d",__LINE__,buf,jpegData,dataLen,buflen);
    ...
    //printf("ret:%x-decode time = %lld us (%lld %lld %lld %lld)\n",ret,finishDecTime-startDecTime, b-a,c-a,d-a,e-a);
    switch (ret)
    {
